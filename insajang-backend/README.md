...
- **Problem**: AI 생성 모델 연동 시 발생하는 API 실패 데이터가 메인 서비스의 콘텐츠 테이블 구조을 오염시키고 DB 롤백 처리를 복잡하게 만드는 정합성 위협이 존재했습니다.
...
- **Action**: 역할을 철저히 분산한 `[React(UI) -> Spring Boot(비즈니스 코어) -> Python FastAPI(비즈니스 AI 가공) -> DB 적재]` 구조 of 마이크로서비스 지향 파이프라인을 설계했습니다. 각각의 서버를 도커 컨테이너로 격리하여 시스템 독립성을 보장했습니다.
